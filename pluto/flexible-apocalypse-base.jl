...
# ╔═╡ faaae3d9-1315-4d9e-b2e9-c0ba43fbe5a2
using JSON
...
The main question was if matches for XXX (i.e., the same digit, repeated) are also less frequent in other bases than 10.

- Result: yes, in all bases tested (3, 5, 7, 10, 11, 13, 17) XXX is clearly less apocalyptic than other sequences
- Result: this pattern is also repeated for other base powers ($x^n$ for $x=3,4,5$)
- Note: The pattern of apocalyptic numbers looks the same for different base powers, which is completely unexpected **double check this** 
- **FALSE** There was a re-running issue somewhere in the notebook, meaning cells were not being correctlt re-run and the same plot was being reproduced, even though the power parameter had changed.
...
begin
	...
	"str-seq-p$(power)-b$(base)-l$(seq_length)-n$(start_n)_$(finish_n).json"), "w") do io
			JSON.print(io, string_sequence, 2)
	end
end

# ╔═╡ 7ac1a8ce-a869-4d0d-a018-e31b8b03c94e
...
# ╔═╡ 6d679443-fd61-433f-a5db-a33cdea84f3d
open(joinpath("..", "results", 
	"n-apocalypse-p$(power)-b$(base)-l$(seq_length)-n$(start_n)_$(finish_n).json"), "w") do io
			JSON.print(io, n_apocalypse, 2)
end
...
savefig(apocalypse_dist, joinpath("..", "results", 
...
savefig(apocalypse_hist, joinpath("..", "results", 
...
manifest_format = "2.0"
project_hash = "19e2b17f43c02a8d57c781821826437028920744"
